...
public class GetNinaUseCasesTwo {
    ...
    public String getFileName() {
        ...
        try {
            s = file.getCanonicalPath() + "\\src\\tools\\getHandler\\ccccoooInfo\\" + sEnd;
...
//            }
    ...
    public void clearTheNotUseCase(String pathNinaUseCase) {
        ArrayList<String> arrayListResult = new ArrayList<String>();
        File newText = new File(pathNinaUseCase.substring(0, pathNinaUseCase.lastIndexOf('\\') + 1) + "NinaCaseFormat.txt");
        File newTextText = new File(pathNinaUseCase.substring(0, pathNinaUseCase.lastIndexOf('\\') + 1) + "NinaCaseFormatFinal.txt");
        try {
            FileInputStream fileInputStream = new FileInputStream(newText);
            InputStreamReader inputStreamReader = new InputStreamReader(fileInputStream);
            BufferedReader bufferedReader = new BufferedReader(inputStreamReader);
            String line = "";
            String sBengin = "[\\d]\\.[\\d]\\.[\\d]";
            if (!newTextText.exists()) {
                newTextText.createNewFile();
            } else {
                FileWriter fileWriterClear = new FileWriter(pathNinaUseCase.substring(0, pathNinaUseCase.lastIndexOf('\\') + 1) + "NinaCaseFormatFinal.txt", false);
                fileWriterClear.write("");
                fileWriterClear.flush();
                fileWriterClear.close();
            }
            ArrayList<String> arrayList = new ArrayList<String>();
            FileWriter fileWriter = new FileWriter(newTextText, true);
            BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
            while ((line = bufferedReader.readLine()) != null) {
                Pattern patternTemp = Pattern.compile(sBengin);
                Matcher matcherTemp = patternTemp.matcher(line);
...
//                    bufferedWriter.write(line+"\n");
    ...
    public static void main(String[] args) {
        ...
        getNinaUseCases.clearTheNotUseCase(getNinaUseCases.getFileName());
    }
}
